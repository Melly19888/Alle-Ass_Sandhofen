...
['StädteQuiz', 'Memory', 'Absenden', 'BilderRaetsel', 'AllgemeineQuiz', 'Neu' , 'Datenlöschen', 'Datenreinigen'].forEach(id => document.getElementById(id).style.display = "none");
...
if(tableDataString){ fetch('send.php',{method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:'tableData='+encodeURIComponent(tableDataString)})
.then(response=>{if(!response.ok){throw new Error('Netzwerkantwort war nicht ok');}return response.json();})
.then(data=>{if(data&&data.success){ console.log('E-Mail erfolgreich gesendet');return fetch('save_to_file.php',{method:'POST', headers:{'Content-Type':'application/json'},body:tableDataString});} else{throw new Error(data.error||'Fehler beim Senden der E-Mail');}})
.then(response=>{if(!response.ok){throw new Error('Netzwerkantwort war nicht ok');}return response.json();})
...
.catch((error)=>{console.error('Fehler:',error);if(error.response){ console.error('Antwortstatus:',error.response.status);error.response.text().then(text=>console.error('Antworttext:',text));}});} else{ console.error("Keine Tabellen-Daten im Local Storage gefunden."); } } 

function clearTableData() { 

localStorage.removeItem('tableData'); 

const tableBody=document.querySelector('#spielerTabelle tbody'); while(tableBody.firstChild)tableBody.removeChild(tableBody.firstChild); } 
...
function addPlayerToTable(playerName) { const tableBody=document.querySelector("#spielerTabelle tbody")||
document.querySelector("#spielerTabelle"); 
let playerExists=false; for(let row of tableBody.rows){
	if(row.cells[0].textContent===playerName){ 
	playerExists=true; break;} } if(playerExists){
		showCustomPopup("Spielername schon vorhanden, wähle einen anderen");
		...
		}else{ let newRow=tableBody.insertRow(); 
		['name','punkte','zeit'].forEach((cls,i)=>{ 
		let cell=newRow.insertCell(i); cell.className='spieler-'+cls; cell.textContent=i===0?playerName:''; }); saveTableData();} }

function startTableUpdateInterval(){ setInterval(updatePlayerData,3000);// Alle3Sekundenaktualisieren
 }

function saveTableData(){ const rows=[...document.querySelector('#spielerTabelle tbody').rows]; 
localStorage.setItem( 'tableData',JSON.stringify( rows.map(r=>({name:r.cells[0].textContent,points:r.cells[1].textContent,time:r.cells[2].textContent})) 
) ); }
